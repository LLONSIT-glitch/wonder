...
def compare_pointers(version: str) -> dict[Path, BssSection]:
    mapfile_path = Path(f"build/{version}/wonderprojectj2.jp.map")
    base_path = Path(f"baserom.jp.z64")
    build_path = Path(f"build/wonderprojectj2.jp.z64")
    ...
    for mapfile_segment in mapfile:
        if not (
            ...
            or mapfile_segment.name.startswith(".main")
            or mapfile_segment.name.startswith(".main_bss")
        ):
            continue
