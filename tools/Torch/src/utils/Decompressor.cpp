...
DataChunk* Decompressor::Decode(const std::vector<uint8_t>& buffer, const uint32_t offset, const CompressionType type, bool ignoreCache) {
    ...
    switch (type) {
        ...
        case CompressionType::SPI1:
        case CompressionType::SPIN:
        case CompressionType::SPI0: {
            SPI_TYPE spiType;

            if (type == CompressionType::SPI0) {
                spiType = SPI_0;
            } else if (type == CompressionType::SPI1) {
                spiType = SPI_1;
            } else {
                spiType = SPI_N;
            }

            SpiHeader* header = (SpiHeader*)&in_buf[0];
            spi_swap_header(header);

            SPDLOG_INFO("SPI Header after swap");
            SPDLOG_INFO("fileSize: {:X}", header->fileSize);
            SPDLOG_INFO("unk8: {:X}", header->unk8);
            SPDLOG_INFO("unkC: {:X}", header->unkC);

            uint8_t* compressedData = (uint8_t*)in_buf + sizeof(SpiHeader);
            uint8_t* decompressed = spi_decompress(spiType, compressedData, header->fileSize, header->unk8, header->unkC);

            if (decompressed == NULL) {
                throw std::runtime_error("Failed to decode SPI block");
            }

            gCachedChunks[offset] = new DataChunk{ decompressed, (uint32_t)header->fileSize };
            return gCachedChunks[offset];
        }
        default:
...
DecompressedData Decompressor::AutoDecode(YAML::Node& node, std::vector<uint8_t>& buffer, std::optional<size_t> manualSize) {
    ...
    switch(type) {
        ...
        case CompressionType::SPI1: // Ignore autodecode for SPI1
...
CompressionType Decompressor::GetCompressionType(std::vector<uint8_t>& buffer, const uint32_t offset) {
    if (offset) {
        ...
        if (header == "SPI1") {
            SPDLOG_ERROR("got spi1 header in offset: {:X}", offset);
            return CompressionType::SPI1;
        }

        if (header == "SPI0") {
            SPDLOG_ERROR("got spi0 header?");
            return CompressionType::SPI0;
        }

        if (header == "SPIN") {
            SPDLOG_ERROR("got spin header?");            
            return CompressionType::SPIN;
        }
    }
