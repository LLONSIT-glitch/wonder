...
static const std::unordered_map <std::string, CompressionType> sCompressionTypes = {
    ...
    { "SPI1", CompressionType::SPI1 },
    { "SPI0", CompressionType::SPI0 },
    { "SPIN", CompressionType::SPIN },
};
...
ExportResult CompressedTextureCodeExporter::Export(std::ostream &write, std::shared_ptr<IParsedData> raw, std::string& entryName, YAML::Node &node, std::string* replacement) {
    ...
    switch (texture->mCompressionType) {
        ...
        case CompressionType::SPIN: {
            ...
            if (texture->mCompressionType == CompressionType::SPI0) {
                spiType = SPI_0;
            }
                    
            else if (texture->mCompressionType == CompressionType::SPI1) {
                spiType = SPI_1;
            } else {
                spiType = SPI_N;
            }
                    
            worstSize = SPI_HEADER_LENGTH + ((data.size() + 7) / 8) + data.size();
            compressedData = static_cast<uint8_t *>(std::calloc(worstSize, sizeof(uint8_t)));
            uint8_t *compressed = spi_compress(data.data(), (int)data.size(), spiType, (int *)&compressedSize);
            memcpy(compressedData, compressed, compressedSize);
            break;
        }
        default:
            // UNIMPLEMENTED
